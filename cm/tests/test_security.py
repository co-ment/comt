...
class SecurityTest(TestCase):
    ...
    def test_moderation_tricks_a_priori(self):
        ...
        self.assertTrue(has_own_perm(FakeRequest(user2), "can_edit_comment" + "_own", text2, c2),
                        'can edit own comment')

        c3 = create_comment(user=user3)
        self.assertTrue(has_own_perm(FakeRequest(user3), "can_edit_comment" + "_own", text2, c3),
                        'can edit own comment')
        ...
        self.assertFalse(has_own_perm(FakeRequest(user3), "can_edit_comment" + "_own", text2, c3),
                         'CANNOT edit own comment (there is a reply)')
        self.assertTrue(has_own_perm(FakeRequest(user2), "can_edit_comment" + "_own", text2, c2),
                        "CAN edit own comment (is moderator)")
        self.assertTrue(has_perm(FakeRequest(user2), "can_edit_comment", text2),
                        "CAN edit other comment (is moderator)")
    ...
    def test_moderation_tricks_a_posteriori(self):
        ...
        self.assertTrue(has_own_perm(FakeRequest(user3), "can_edit_comment" + "_own", text2, c3),
                        'CAN edit own comment (there is NO reply)')
        ...
        self.assertFalse(has_own_perm(FakeRequest(user3), "can_edit_comment" + "_own", text2, c3),
                         'CANNOT edit own comment (there is a reply)')
        self.assertTrue(has_perm(FakeRequest(user2), "can_edit_comment", text2),
                        "CAN edit other's comment (moderator)")
