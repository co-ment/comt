...
from cm.models import UserRole, UserProfile, Role, ApplicationConfiguration
            ...
            if match:
                ...
                for user_profile_key in user_profile_keys:
                    user_role = UserRole.objects \
                        .get(user__userprofile__key=user_profile_key, text=None)
        ...
        if 'save' in request.POST:
            ...
            for user_profile_key in user_profile_keys_roles:
                ...
                if not user_profile_key:
                    ...
                    user_role = UserRole.objects.get(user__userprofile__key=user_profile_key,
                                                     text=None)
...
def user_add(request, key=None, mass=False):
    ...
    if request.method == 'POST':
        ...
        if userform.is_valid() \
                and (not userroleform or userroleform.is_valid()) \
                and noteform.is_valid() \
            ...
            for email in [s.strip() for s in SEPARATORS_RE.split(emails)]:
                if email and not User.objects.filter(email__iexact=email) \
                        and email not in email_created:
                    user = UserProfile.objects.create_inactive_user(email, True, **data)
                    if key:
                        localuserrole = UserRole.objects.create(user=user,
                                                                role=localroleform.cleaned_data['role'],
                                                                text=text)
                    else:
                        userrole = UserRole.objects.create(user=user,
                                                           role=userroleform.cleaned_data['role'],
                                                           text=None)
                    email_created.add(email)
                    register_activity(request, "user_created", user=user)
            display_message(request,
                            ungettext(u'%(nb_users)d user added', u'%(nb_users)d users added',
                                      len(email_created)) % {'nb_users': len(email_created)})
            if key:
    ...
    template_ctx = {
        'forms': [userform, userprofileform, userroleform, noteform,
                  localroleform],
...
def profile_pw(request):
    ...
    template_ctx = {'forms': [pwform], 'title': 'Password', }
...
def user_anon_edit(request):
    ...
    template_ctx = {'form': userroleform, 'title': 'Edit anonymous user', }
...
def user_contact(request, key):
    ...
    template_ctx = {
        'form': contact_form, 'save_name': 'send',
