...
from cm.models import Permission, UserRole, Text, Activity, Comment
from cm.exception import UnauthorizedException
from cm.cm_settings import DECORATED_CREATORS, NO_SECURITY
...
def has_perm(request, perm_name, text=None):
    ...
    try:
      ...
      myself = None
    key = sha1(str((settings.SITE_URL,
                    'has_perm',
                    (user, myself, text, perm_name)))).hexdigest()
    val = cache.get(key)
    if val != None:
      return val
    ...
    if not text:
        ret = UserRole.objects.filter(user=user, text=None) \
                  .filter(Q(role__permissions__codename__exact=perm_name)) \
                  .count() != 0
        cache.set(key, ret)
        return ret
    else:
        ...
        if UserRole.objects.filter(Q(user=user), Q(text=text), ~Q(role=None)): # if non void local role
            ...
            ret = UserRole.objects \
                      ...
                      .filter(Q(text=None) | Q(text=text)) \
                      .filter(Q(role__permissions__codename__exact=perm_name)) \
                      .count() != 0
            cache.set(key, ret)
            return ret
...
def has_own_perm(request, perm_name, text, comment):
    ...
    key = sha1(str((settings.SITE_URL,
                    'has_own_perm',
                    (user, myself, text, comment, perm_name)))).hexdigest()
    val = cache.get(key)
    if val != None:
        return val
    ...
    if comment and comment.state == 'approved' \
            and perm_name == 'can_edit_comment_own' \
            and text.last_text_version.mod_posteriori == False \
            and  not has_perm(request, 'can_manage_text', text=text):
        cache.set(key, False)
        return False
...
def get_texts_with_perm(request, perm_name):
    ...
    # local role OVERRIDES global role:
    texts_with_local_role = Text.objects \
        .filter(userrole__in=UserRole.objects.filter(Q(user=user) | Q(user=None)).filter(~Q(role=None)))
    ...
    texts_with_local_role_with_perm = Text.objects \
        .filter(id__in=texts_with_local_role) \
        .filter(Q(userrole__role__permissions__codename__exact=perm_name),
                Q(userrole__user=user) | Q(userrole__user=None)) \
        .distinct()
    
    # global perm?
    if UserRole.objects.filter(text=None)\
            .filter(role__permissions__codename__exact=perm_name) \
            .filter(Q(user=user) | Q(user=None) ).count() != 0:
        texts_without_local_role_with_perm \
            = Text.objects.filter(id__in=texts_without_local_role)
    else:
        texts_without_local_role_with_perm = []
    
    ids = set([t.id for t in texts_with_local_role_with_perm]) \
        .union(set([t.id for t in texts_without_local_role_with_perm]))
...
def get_viewable_comments(request, comments, text, order_by=('created',)):
    ...
    if user and has_perm(request, 'can_view_unapproved_comment', text=text):
        ...
        if has_perm(request, 'can_view_approved_comment', text=text):
            ...
            comments_thread_viewable = [c for c in visible_comments
                                        if c.is_thread_full_visible()]
            ...
            if DECORATED_CREATORS:
                visible_comments = comments.filter(name=myself) \
                    .order_by(*order_by)
            else:
                visible_comments = comments.filter(user=user) \
                    .order_by(*order_by)
            ...
            if (role_model == 'teacher'):
                ...
                if DECORATED_CREATORS:
                    visible_comments = comments \
                        .filter(Q(user__id__in=with_teachers) | Q(name=myself)) \
                        .order_by(*order_by)
                else:
                    with_teachers.append(user.id)
                    visible_comments = comments \
                        .filter(user__id__in=with_teachers).order_by(*order_by)
...
def list_viewable_comments(request, comments_list, text):
    ...
    for comment in comments_list :
        ret += [comment] + list_viewable_comments(request,
                                                  get_viewable_comments(request, comment.comment_set, text),
                                                  text)
    return ret
...
def has_perm_on_text_api(perm_name, must_be_logged_in=False,
                         redirect_field_name=REDIRECT_FIELD_NAME):
    return _has_perm_on_text(perm_name, must_be_logged_in, redirect_field_name,
                             api=True)


def has_perm_on_text(perm_name, must_be_logged_in=False,
                     redirect_field_name=REDIRECT_FIELD_NAME, api=False):
    return _has_perm_on_text(perm_name, must_be_logged_in, redirect_field_name,
                             api)


def _has_perm_on_text(perm_name, must_be_logged_in=False,
                      redirect_field_name=REDIRECT_FIELD_NAME, api=False):
    """
...
def has_global_perm_or_perm_on_text(global_perm_name, perm_name,
    ...
    def _dec(view_func):
        def _check_global_or_local_perm(request, *args, **kwargs):
            ...
            raise UnauthorizedException(
                'No global perm %s nor local perm %s' % (
