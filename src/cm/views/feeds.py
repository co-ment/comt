...
def get_tag_uri(url, date):
    ...
    return u'tag:%s,%s:%s' \
           %(parsed_url.hostname, time_stp, parsed_url.path) # + '/' + parsed_url.fragment


def public_feed(request):
    workspace_name = ApplicationConfiguration['workspace_name']
    feed_title = _(u"%(workspace_name)s's public feed") \
                 % {'workspace_name': workspace_name}
    feed_description = _(u"Workspace %(workspace_name)s public activity feed") \
                       % {'workspace_name': workspace_name}
    request.user = AnonymousUser()
    activitites = get_viewable_activities(request,
                                          {'view_comments': 1, 'view_texts': 1})
...
def private_feed(request, key):
    ...
    feed_title = _(u"%(workspace_name)s's private feed"
                   % {'workspace_name' : ApplicationConfiguration['workspace_name']})
    feed_description = _(u"Workspace %(workspace_name)s private feed")\
                       % {'workspace_name' : ApplicationConfiguration['workspace_name']}
    activities = Activity.objects \
        .filter(type__in=Activity.ACTIVITIES_TYPES).order_by('-created')
...
def _feed(request, activities, feed_title, feed_description):
    ...
    for activity in activities:
        ...
        feed.add_item(title=item_data_text,
                      ...
                      unique_id=get_tag_uri(settings.SITE_URL,
                                            activity.created),
...
def text_feed_private(request, key, private_feed_key):
    ...
    feed_description = _(u"Private activity feed for text %(text_title)s") \
                       % {'text_title': text.title}
    activities = Activity.objects \
        .filter(type__in=Activity.ACTIVITIES_TYPES, text=text) \
        .order_by('-created')
    return _feed(request, activities, feed_title, feed_description)
